...
public class InstantiateSeal : MonoBehaviour {
	...
	void Start () {
		...
		myselfButton.onClick.AddListener(() => instantiateMySeal());
	}

	void instantiateMySeal() {
	...
	void Destroy() {
		myselfButton.onClick.RemoveListener(() => instantiateMySeal());
	}
}